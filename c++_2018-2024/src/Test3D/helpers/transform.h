...
namespace Math {
    ...
    template <CoordinateSystem::Enum _system = CoordinateSystem::Current>
    TransformMatrix<_system> fromPositionScaleAndRotationEulers(const Vec3& position, f32 scale, const Vec3& rotationEulers);
    template <CoordinateSystem::Enum _system = CoordinateSystem::Current>
...
namespace Math {
    ...
    template <>
    TransformMatrix<CoordinateSystem::RH_Zup> fromPositionScaleAndRotationEulers<CoordinateSystem::RH_Zup>(const Vec3& position, f32 scale, const Vec3& rotationEulers) {
       
        TransformMatrix<CoordinateSystem::RH_Zup> t;

        Vec4 toRotation = Math::eulersToQuaterion(rotationEulers);
        Mat33 rotation = Math::quaternionToRotationMatrix(toRotation);
