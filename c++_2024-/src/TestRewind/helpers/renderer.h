...
namespace Renderer {
    ...
    template<>
    void generate_matrix_ortho<ProjectionType::Zminus1to1>(float4x4& matrix, const WindowProjection::Config& config) {
    ...
    template<>
    void generate_matrix_ortho<ProjectionType::Z0to1>(float4x4& matrix, const WindowProjection::Config& config) {
        f32* matrixCM = matrix.dataCM;
        memset(matrixCM, 0, sizeof(f32) * 16);
        matrixCM[0] = 2.f / (config.right - config.left);
        matrixCM[5] = 2.f / (config.top - config.bottom);
    ...
    template <>
    void generate_matrix_persp<ProjectionType::Zminus1to1>(float4x4& matrixRHwithYup, const PerspProjection::Config& config) {
    ...
    template <>
    void generate_matrix_persp<ProjectionType::Z0to1>(float4x4& matrixRHwithYup, const PerspProjection::Config& config) {
        const f32 h = 1.f / Math::tan(config.fov * 0.5f * Math::d2r_f);
        const f32 w = h / config.aspect;
    ...
    void create_textured_cube_coords(TexturedCube& c, const CubeCreateParams& params) {
        ...
        f32 h = params.height;
        f32 d = params.depth;
        Renderer::Layout_float3TexturedMapped* v = c.vertices;
	...
	void create_untextured_cube_coords(UntexturedCube& c, const CubeCreateParams& params) {
		...
		f32 h = params.height;
		f32 d = params.depth;
		Renderer::Layout_float3* v = c.vertices;
...
void create_indexed_vertex_buffer_from_untextured_cube(Renderer::Driver::RscIndexedBuffer<Renderer::Layout_float3>& buffer, const CubeCreateParams& params) {
...
void create_indexed_vertex_buffer_from_textured_cube(Renderer::Driver::RscIndexedBuffer<Renderer::Layout_float3TexturedMapped>& buffer, const CubeCreateParams& params) {
...
namespace FBX {
    void extract_vertex(Renderer::Layout_float3& vertexout, const ufbx_float3& vertexin) {
        vertexout.x = vertexin.x;
        vertexout.y = vertexin.y;
        vertexout.z = vertexin.z;
    }
    void extract_vertex(Renderer::Layout_float3Color4B& vertexout, const ufbx_float3& vertexin) {
        vertexout.pos.x = vertexin.x;
        vertexout.pos.y = vertexin.y;
        vertexout.pos.z = vertexin.z;
    }
    void extract_vertex(Renderer::Layout_float3TexturedMapped& vertexout, const ufbx_float3& vertexin) {
        vertexout.pos.x = vertexin.x;
        vertexout.pos.y = vertexin.y;
        vertexout.pos.z = vertexin.z;
    }
    void extract_vertex_attrib(Renderer::Layout_float3* vertices, const ufbx_mesh& mesh) {}
    void extract_vertex_attrib(Renderer::Layout_float3Color4B* vertices, const ufbx_mesh& mesh) {
    ...
    void extract_vertex_attrib(Renderer::Layout_float3TexturedMapped* vertices, const ufbx_mesh& mesh) {
        if (mesh.vertex_uv.exists && mesh.vertex_normal.exists && mesh.vertex_tangent.exists && mesh.vertex_bitangent.exists) {
            for (size_t index = 0; index < mesh.num_indices; index++) {
                ...
                vertices[mesh.vertex_indices[index]].uv = float2(uv.x, uv.y);
                vertices[mesh.vertex_indices[index]].normal = float3(normal.x, normal.y, normal.z);
                vertices[mesh.vertex_indices[index]].tangent = float3(tangent.x, tangent.y, tangent.z);
                vertices[mesh.vertex_indices[index]].bitangent = float3(bitangent.x, bitangent.y, bitangent.z);
            }
        }
    }
