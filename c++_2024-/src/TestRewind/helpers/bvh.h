...
namespace BVH {
	...
	void buildTreeRec(Tree& bvh, const u32 nodeId, const BuildTreeContext& context, const tinystl::vector<u32>& triangleIds) {
		...
		if (bvh.nodes[nodeId].isLeaf) {
			...
			float3 currExtents = Math::subtract(bvh.nodes[nodeId].max, bvh.nodes[nodeId].min);
			u8 widestCoord = 0;
	...
	bool queryRayIntersectsWithTriangle(f32& t, const float3& p, const float3& dir, const float3& a, const float3& b, const float3& c) {
		...
		float3 ba = Math::subtract(b, a);
		float3 ca = Math::subtract(c, a);
		float3 pa = Math::subtract(p, a);
		float3 n = Math::cross(ba, ca);
		float3 q = Math::cross(pa, dir);
	...
	bool queryRayIntersectsWithBox(f32& t, const float3& p, const float3& dir, const float3& min, const float3& max) {
	...
	bool queryIsPointInside(tinystl::vector<f32>& closestTs, const Tree& bvh, const float3& p, const float3& biasDir, f32* vertexPool, const u16* indexPool, const u32 indexCount) {
	...
	void closestPointOnTriangleSq(float3& closestPoint, f32& closestDistSq, const float3& p, const float3& a, const float3& b, const float3& c) {
		float3 ba = Math::subtract(b, a);
		float3 pa = Math::subtract(p, a);
		float3 cb = Math::subtract(c, b);
		float3 pb = Math::subtract(p, b);
		float3 ac = Math::subtract(a, c);
		float3 pc = Math::subtract(p, c);
		float3 n = Math::cross(ba, ac);
		...
		if (insidePrism) {
			...
			float3 pp_ba = Math::subtract(Math::scale(ba, Math::clamp(Math::dot(ba, pa) / Math::dot(ba, ba), 0.f, 1.f)), pa);
			f32 distanceSqBA = Math::dot(pp_ba, pp_ba);
			float3 pp_cb = Math::subtract(Math::scale(cb, Math::clamp(Math::dot(cb, pb) / Math::dot(cb, cb), 0.f, 1.f)), pb);
			f32 distanceSqCB = Math::dot(pp_cb, pp_cb);
			float3 pp_ac = Math::subtract(Math::scale(ac, Math::clamp(Math::dot(ac, pc) / Math::dot(ac, ac), 0.f, 1.f)), pc);
	...
	f32 distanceToBoxSq(const float3& p, const float3& min, const float3& max) {
		float3 clamped = Math::max(Math::min(max, p), min);
	...
	void findClosestPoint(float3& resultPoint, const Tree& bvh, const float3& p, f32* vertexPool, const u16* indexPool, const u32 indexCount) {
