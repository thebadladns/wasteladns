...
namespace Renderer {
    ...
    typedef Shader_Params<
        ...
        , Renderer::Layout_Texturedfloat3, Layout_CNone, Layout_CNone
    ...
    namespace FBX {
        ...
        typedef DrawlistSrcSelector<Layout_float3Color4B, Drawlist::DL_color_t> DL_color_t;
        typedef DrawlistSrcSelector<Layout_float3Color4BSkinned, Drawlist::DL_colorskinned_t> DL_colorskinned_t;
        typedef DrawlistSrcSelector<Layout_TexturedSkinnedfloat3, Drawlist::DL_textured_t> DL_textured_t;
        typedef DrawlistSrcSelector<Layout_TexturedSkinnedfloat3, Drawlist::DL_texturedalphaclip_t> DL_texturedalphaclip_t;
        ...
        template<>
        void extract_vertex_attrib<Layout_Texturedfloat3, DstDataType::Vertex>(Layout_Texturedfloat3& vertex, const ufbx_mesh& mesh, const ufbx_mesh_material&, const u32 index_id, const u32 vertex_id) {
            ufbx_float2& uv = mesh.vertex_uv[index_id];
            vertex.uv.x = uv.x;
            vertex.uv.y = uv.y;
        }
        template<>
        void extract_vertex_attrib<Layout_TexturedSkinnedfloat3, DstDataType::Vertex>(Layout_TexturedSkinnedfloat3& vertex, const ufbx_mesh& mesh, const ufbx_mesh_material&, const u32 index_id, const u32 vertex_id) {
            ufbx_float2& uv = mesh.vertex_uv[index_id];
            vertex.uv.x = uv.x;
            vertex.uv.y = uv.y;
                            ...
                            if (mesh_mat.material && mesh_mat.material->pbr.base_color.has_value) {
                                if (mesh_mat.material->pbr.base_color.texture_enabled) {
                                    ...
                                    if (mesh_mat.material->pbr.opacity.texture_enabled) {
                                        ...
                                        process_material<Layout_TexturedSkinnedfloat3, DstDataType::Vertex>(mesh_vertices, mesh_vertices_count, streamTexturedSkinnedAlphaClip.streams, mesh, mesh_mat, scratchArena);
                                        ...
                                        process_material<Layout_TexturedSkinnedfloat3, DstDataType::Vertex>(mesh_vertices, mesh_vertices_count, streamTexturedSkinned.streams, mesh, mesh_mat, scratchArena);
                                        streamTexturedSkinned.user = mesh_mat.material->pbr.base_color.texture;
                                    }
                                } else {
                                    process_material<Layout_float3Color4BSkinned, DstDataType::Material>(mesh_vertices, mesh_vertices_count, streamColorSkinned.streams, mesh, mesh_mat, scratchArena);
                                }
                            } else {
                                if (mesh.vertex_color.exists) {
                                    process_material<Layout_float3Color4BSkinned, DstDataType::Vertex>(mesh_vertices, mesh_vertices_count, streamColorSkinned.streams, mesh, mesh_mat, scratchArena);
                                } else {
                                    process_material<Layout_float3Color4BSkinned, DstDataType::Dummy>(mesh_vertices, mesh_vertices_count, streamColorSkinned.streams, mesh, mesh_mat, scratchArena);
                                }
                            }
                        } else {
                            if (mesh_mat.material && mesh_mat.material->pbr.base_color.has_value) {
                                process_material<Layout_float3Color4B, DstDataType::Material>(mesh_vertices, mesh_vertices_count, streamColor.streams, mesh, mesh_mat, scratchArena);
                            } else if (mesh.vertex_color.exists) {
                                process_material<Layout_float3Color4B, DstDataType::Vertex>(mesh_vertices, mesh_vertices_count, streamColor.streams, mesh, mesh_mat, scratchArena);
                            } else {
                                process_material<Layout_float3Color4B, DstDataType::Dummy>(mesh_vertices, mesh_vertices_count, streamColor.streams, mesh, mesh_mat, scratchArena);
                            }
                        }
                    }
